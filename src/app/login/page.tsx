...
export default function LoginPage() {
  ...
  const params = typeof window !== 'undefined'
    ...
    : null;
  const urlError = params?.get('error') === 'invalid'
  ...
  const handleGoogle = () => {
    toast('Google OAuth coming soon', {
      description: 'Use email magic link for now.',
    });
  };
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[var(--color-bg)] px-4">
      ...
      <div className="w-full max-w-sm">
        ...
        {urlError && (
          <div className="mb-4 rounded-xl border border-red-100 bg-red-50/60 px-4 py-3 text-sm text-red-500">
            {urlError}
          </div>
        )}
        ...
        {!sent ? (
          <div className="rounded-2xl border border-black/8 bg-white p-8 shadow-sm">
            ...
            <button
              ...
              className="w-full rounded-xl border border-black/10 bg-white py-3 text-sm font-medium text-black/70 transition-all hover:bg-black/3 flex items-center justify-center gap-2.5"
            >
              <svg
                width="16"
                height="16"
