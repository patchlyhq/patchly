...
function dbToUi(e: DbEntry): Entry {
...
function EditEntryDialog({
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/projects/${window.location.pathname.split('/')[3]}/entries/${entry.id}`, {
        method: 'PATCH',
          ...
          {open && (
            <>
              ...
              <Dialog.Content asChild>
                <motion.div
                  className="fixed left-1/2 top-[8%] z-50 w-full max-w-xl -translate-x-1/2 rounded-2xl border border-black/10 bg-white p-6 shadow-[0_20px_60px_rgba(0,0,0,0.12)]"
...
export default function ProjectPage({
  ...
  useEffect(() => {
    fetch(`/api/projects/${slug}/entries`)
      .then((r) => r.json())
      .then((data: DbEntry[]) => setEntries(data.map(dbToUi)))
    ...
    <div className="max-w-4xl">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-24">
          <Loader2 size={18} className="animate-spin text-black/25" />
        </div>
      ) : entries.length === 0 ? (
                  ...
                  <td className="hidden px-5 py-3.5 sm:table-cell">
                    <div className="flex flex-wrap gap-1.5">
                      {entry.tags.map((tag) => (
                        <span
                          key={tag}
                          className="rounded border border-black/10 px-2 py-0.5 text-xs text-black/40"
                        >
                          {tag}
                        </span>
                      ))}
